...
export function LoadCard({ load, searchQuery = "" }: LoadCardProps) {
  ...
  const handleWhatsApp = () => {
    ...
    const fromText = load.from && load.from !== "-" ? load.from : "Belirtilmedi"
    const toText = load.to && load.to !== "-" ? load.to : "Belirtilmedi"
    
    const message = `Merhaba, ${fromText} -> ${toText} güzergahındaki (${load.cargo || 'Yük'}) ilanınız için Nakliye Cepte üzerinden iletişime geçiyorum.`
  ...
  const hasFrom = load.from && load.from !== "-"
  const hasTo = load.to && load.to !== "-"
  ...
  return (
    <div className="group relative flex flex-col justify-between rounded-xl border border-[#e4e9ef] bg-white p-5 shadow-sm transition-all duration-200 hover:border-[#cbd5e1] hover:shadow-md">
      <div>
        {/* Üst Kısım: Firma Bilgisi, Kaynak ve Zaman */}
        <div className="flex items-center justify-between pb-3.5 border-b border-[#f0f4f8]">
          <div className="flex items-center gap-3 min-w-0">
            <div className={`grid size-10 shrink-0 place-items-center rounded-xl text-white font-bold text-sm shadow-xs ${load.color || (load.source === "user" ? "bg-[#d64526]" : "bg-[#315d83]")}`}>
              {load.initials || "LN"}
            </div>
            <div className="min-w-0 truncate">
              ...
              <div className="text-xs text-[#8da0b2] flex items-center gap-1 mt-0.5">
                ...
                <span className="truncate">
                  {load.source === "bot" ? "WhatsApp Bot İlanı" : "Nakliye Cepte Kullanıcısı"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <span className="font-bold text-[#122c4a] text-sm sm:text-base truncate">
                  {hasFrom ? highlightMatch(load.from, cleanQuery) : "Belirtilmedi"}
                </span>
              </div>
            </div>
            ...
            <div className="flex-1 min-w-0 text-right">
              ...
              <div className="flex items-center justify-end gap-1.5 mt-0.5">
                <span className="font-bold text-[#122c4a] text-sm sm:text-base truncate">
                  {hasTo ? highlightMatch(load.to, cleanQuery) : "Belirtilmedi"}
                </span>
        ...
        <div className="space-y-2 mb-3">
          ...
          {/* Varsa Özel Açıklama / Mesaj Notu */}
      ...
      {/* Alt Alt Bölüm: İletişim ve Aksiyon Butonları */}
